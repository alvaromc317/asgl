...
if __name__ == '__main__':
    ...
    # UNPENALIZED TEST
    tvt_class = cv.TVT(model='lm', penalization=None, error_type='MSE', random_state=2, train_pct=0.7, validate_pct=0.1)

    cv1 = tvt_class.train_validate_test(x, y, group_index)
    cv2 = tvt_class.train_validate_test(x, y, group_index)
    equal_array(cv1[0], cv2[0])

    # LASSO penalization
    tvt_class = cv.TVT(model='qr', penalization='lasso', lambda1=[0.01, 0.1, 1, 10], parallel=True, tau=0.5,
                       error_type='QRE', random_state=1, train_pct=0.3, validate_pct=0.6)
    tvt_class.train_validate_test(x, y, group_index)

    # Group LASSO
    tvt_class = cv.TVT(model='lm', penalization='gl', lambda1=[0.01, 0.1, 1, 10], parallel=True,
                       error_type='MAE', random_state=2, train_pct=0.1, validate_pct=0.1)
    tvt_class.train_validate_test(x, y, group_index)

    # Sparse Group LASSO
    tvt_class = cv.TVT(model='qr', penalization='sgl', lambda1=[0.01, 0.1, 1, 10], parallel=True, tau=0.02,
    ...
    tvt_class = cv.TVT(model='qr', penalization='asgl_lasso', lambda1=[0.01, 0.1, 1, 10], tau=0.2,
    ...
    tvt_class = cv.TVT(model='qr', penalization='asgl_gl', lambda1=[0.01, 0.1, 1, 10], tau=0.99,
    ...
    tvt_class = cv.TVT(model='qr', penalization='asgl', lambda1=[0.01, 0.1, 1, 10], tau=0.5,
